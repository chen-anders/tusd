...
func New(bucket string, bucketPath string, service S3API) S3Store {
	return S3Store{
		Bucket:            bucket,
...
func (store S3Store) NewUpload(info tusd.FileInfo) (id string, err error) {
	...
	_, err = store.Service.PutObject(&s3.PutObjectInput{
		Bucket:        aws.String(store.Bucket),
		Key:           store.pathInBucket(uploadId + ".info"),
...
func (store S3Store) pathInBucket(filename string) *string {
	return aws.String(fmt.Sprintf("%s/%s", store.BucketPath, filename))
}
